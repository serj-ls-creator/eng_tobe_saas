...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: success, error: rpcError } = await supabase.rpc('record_learning_progress_secure', {
    ...
    // 2. Fall back to standard in-memory upsert if the RPC function is not deployed yet
    console.warn('RPC record_learning_progress_secure failed or not found, using fallback in-memory logic:', rpcError);
