...
export async function POST(_req: NextRequest) {
  try {
    ...
    // 1. Try calling the atomic, thread-safe database function first
    const { data, error: rpcError } = await supabase.rpc('complete_user_activity', {
    ...
    // 2. Fall back to standard in-memory calculations if the RPC function is not deployed yet
    console.warn('RPC complete_user_activity failed or not found, using fallback in-memory logic:', rpcError);
    ...
    // ── Increment daily activity count (cap at 4) ────────────────────────────
    const prevActivities = dailyActivities;
    if (dailyActivities < 4) {
      dailyActivities += 1;
    }

    // ── Check if day is now complete (crossed 4 activities threshold) ────────
