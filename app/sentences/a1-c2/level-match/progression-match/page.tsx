...
export default async function LevelMatchPage() {
  ...
  // Find the level-match topic
  const topic = category.topics?.find(t => t.id === "level-match");
  if (!topic) notFound();

  // Find the subcategory
  const subcategory = topic.subcategories?.find(s => s.id === "progression-match");
