...
import { wordOrderMistakeLevels } from "@/data/grammar/wordorder/find_the_mistake";
...
export default async function WordOrderFindMistakeLevelsPage() {
  const progress = await getLearningProgressSnapshot();

  return (
    <>
      <TopBar title="Word Order — Find the Mistake" />
      <div className="content-shell pb-8">
        ...
        <div className="space-y-3">
          {wordOrderMistakeLevels.map((lvl, index) => {
            ...
            return (
              <div key={lvl.level} className={`fade-up fade-up-d${Math.min(index + 1, 5)}`}>
                <Link href={`/grammar/word-order/find-the-mistake/level-${lvl.level}`}>
                  <Card
