...
export default async function PairsPage({ params }: { params: { levelId: string } }) {
  ...
  // If levelId is "index" or not provided, show level selection
  if (params.levelId === "index" || !params.levelId) {
    return (
      <>
        <TopBar title="Pairs" />
  ...
  // Otherwise, redirect to the game page with level parameter
  const level = PAIRS_LEVELS.find(l => l.id === params.levelId);
  if (!level) notFound();

  // Redirect to the game page with level parameter
  redirect(`/sentences/a1-c2/pairs/sentence-pairs?level=${params.levelId}`);
}
