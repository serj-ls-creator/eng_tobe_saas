        ...
        <div className="grid grid-cols-2 gap-3">
          {WORD_GAME_ACTIVITIES.map((activity, index) => (
            <div key={activity.id} className={`fade-up fade-up-d${Math.min(index + 1, 5)}`}>
              <Link href={locked ? "/premium" : (activity.id === 'cards' ? `/words/${categoryId}/${topicId}/${subcategoryId}/cards` : activity.id === 'multiple-choice' ? `/words/${categoryId}/${topicId}/${subcategoryId}/multiple-choice` : `/words#${categoryId}-${topicId}-${subcategoryId}-${activity.id}`)}>
                <Card className="p-4">
